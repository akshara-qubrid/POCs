    ...
    def _build_messages(self, instruction: str) -> List[Dict[str, str]]:
        system_message = {
            ...
            "content": (
                ...
                " When you need external analysis, return JSON with 'tool' and 'input'."
                " When complete, return JSON with 'final'."
                f" Available tools:\n{self.state.get_tool_descriptions()}"
            ),
        }
        # Build a properly alternating conversation — only include messages
        # that follow the user/assistant/user/... pattern to avoid API errors.
        ...
        for msg in self.state.history[-20:]:
            role = msg["role"]
            if role == last_role:
                # Skip duplicate-role messages; they violate the alternating constraint
                continue
            alternating.append({"role": role, "content": msg["content"]})
            last_role = role
        messages.extend(alternating)
        # The final user instruction must come after the last assistant turn
        ...
        for iteration in range(MAX_ITERATIONS):
            ...
            if "tool" in action and "input" in action:
                ...
                instruction = (
                    ...
                    "Continue and produce the final JSON output with key 'final'."
                )
                continue
            if "final" in action:
                return action["final"]
