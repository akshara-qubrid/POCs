...
from .llm_client import get_response_text, chat_completion
from .utils import extract_json
from .state import SharedState

MAX_ITERATIONS = 8  # prevent infinite tool-call loops
...
class AgentExecutor:
    def __init__(self, name: str, description: str, state: SharedState, model: str, max_tokens: int = 1024):
        ...
        # Reset on every run() so no bleed between calls and no shared-history injection.
    ...
    def _system_message(self) -> Dict[str, str]:
        return {
            ...
            "content": (
                ...
                " When you need external data, return a JSON object with keys 'tool' and 'input'."
                " When complete, return a JSON object with key 'final'."
    ...
    def _build_messages(self) -> List[Dict[str, str]]:
        """Return [system] + local conversation — always properly alternating."""
    ...
    def run(self, instruction: str) -> Any:
        ...
        for iteration in range(MAX_ITERATIONS):
            ...
            # Also record in shared state for cross-agent observability (not fed back to LLM).
            ...
            if "tool" in action and "input" in action:
                ...
                # Add tool result as a user turn to keep the conversation alternating.
                tool_msg = (
            ...
            if isinstance(action, dict):
                return action
            raise ValueError(f"[{self.name}] LLM response did not contain a valid 'tool' or 'final' key: {text[:200]}")
